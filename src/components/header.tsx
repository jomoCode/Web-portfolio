  ...
  "w-[7.5rem] py-2 text-lg   text-center hover:text-xl  transition-all duration-300 ease-in-out",
  {
    variants: {
      theme: {
        ...
        dark: "bg-background-dark-2 text-text-dark hover:text-slate-400",
      },
    },
    defaultVariants: {
      theme: "dark",
    ...
    <HeaderContainer>
      <ul className="hidden md:flex md:flex-row gap-4">
        {headerContent.map((data, index) => (
          <li key={index} className={headerVariants({ theme })}>
            {data}
          </li>
        ))}
      </ul>
      ...
      <div className="flex items-center gap-2 md:hidden">
        <Bars4Icon className={`h-6 w-6 ${theme ==="dark"?"text-text-dark":"text-text-light"} `} />
        <Text>JomoCode</Text>
      </div>
    </HeaderContainer>
  );
